...
if __name__ == '__main__':
    ...
    def write_csv(training_strategy: str = 'vanilla'):
        ...
        # ic(train_config2csv_path)
        # # Use the dict as a set, enforce row ordering in output
        # # for conf in train_config2csv_path.keys():
        # #     if conf[2] == training_strategy:
        # #         ic(conf)
        setups = OrderedDict([
            (conf[:2], None) for conf in train_setup2csv_path.keys() if conf[2] == training_strategy
        ])
        # ic(setups)
        # setups = [setup for setup in train_setup2csv_path if setup[2] == training_strategy]
        # ic(setups)
        # for train_config in train_setup2csv_path:
        #     model_name, sampling_strategy, training_strategy, domain = train_config
        #     if (model_name, sampling_strategy, training_strategy) in set_configs:
        #         ic(train_config)
        ...
        # ic(training_strategy)

        for (model_name, sampling_strategy) in setups:
            row = [model_name, sampling_strategy]
            key = [model_name, sampling_strategy, training_strategy]
            key_in, key_out = tuple([*key, 'in']), tuple([*key, 'out'])
            # if 'gpt2' in model_name:
            #     ic(model_name, sampling_strategy, key_in, key_out, key_in in train_setup2csv_path)
